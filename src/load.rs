    ...
    Self: Sized,
{
    ...
    fn from_lock_data(&self, lock_data: Self::LockData) -> Result<Self::Target, Self::Error>;
    ...
    fn load_core(
        &self,
        lock_data_path: &Path,
        create: impl Fn(&Self) -> Result<(Self::Target, Self::LockData), Self::Error>,
    ...
    fn load(&self, lock_data_path: &Path) -> Result<Self::Target, Self::Error> {
    ...
    fn load_host(&self, lock_data_path: &Path) -> Result<<Self as Load>::Target, <Self as Load>::Error>
    where
        Self: Host<LockData = <Self as Load>::LockData, Target = <Self as Load>::Target, Error = <Self as Load>::Error>,
    {
...
pub trait Host {
    ...
    /// Provides access, where possible, to the installed version of the system or to the latest version of the system for that platform. It also returns data for subsequent system playback on any supported device.
    fn get_host(&self) -> Result<(Self::Target, Self::LockData), Self::Error>;
}
